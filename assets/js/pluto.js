...
class PlutoComponent extends HTMLElement {
		...
		for (let r in props) {
			if (typeof props[r] === "object") {
				for (let t in props[r]) {
					this.PlutoSW[r] = {};
					this.PlutoSW[r][t] = props[r][t];
				}
			} else {
				this.PlutoSW[r] = props[r];
			}
		}
	...
	off(ev, func) {
		if (typeof ev == "string") {
			...
			Object.keys(ev).forEach((event) => {
				this.removeEventListener(event, ev[event]);
			});
		}
		return this;
	}
	...
	query: (query, scope = document) => {
		var query =
			typeof query === "string"
				...
				: query.nodeType && query.nodeType === Node.ELEMENT_NODE
				? query
				: query instanceof Pluto
				? query.element
				: null;
...
window.PlutoNSSupport.forEach((e) => {
	...
	Object.defineProperty(Pluto, name, {
		get() {
			if (PlutoImplements[name]) {
				var element = new PlutoElement(name, ns);
				return element.props(PlutoImplements[name]);
			}
...
class PlutoElement {
	...
	on(ev, fn) {
		...
		if (typeof ev == "string") {
			...
			Object.keys(ev).forEach((event) => {
				this.element.addEventListener(event, ev[event].bind(event,this));
			});
		}
		return this;
	}
